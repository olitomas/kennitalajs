const Kennitala = (kennitala) => {
    let type, currentRadtala;

    const run = (kennitala) => {
        let kt;
        if ((kennitala.toString() || '').toLowerCase() === 'fake') {
            kt = createFakeKennitala();
        } else {
    ...
    const createFakeKennitala = () => {
        ...
        currentRadtala = parseInt(_7_8, 10);
        let vartala = calculateVartala(list);
        ...
        let _10 = ['0', '8', '9'][randomInt(0, 2)];

        return _1_2 + _3_4 + _5_6 + _7_8 + _9 + _10;
    }

    const existanceInYears = (birthday) => {
        var today = new Date();
    ...
    const getAge = (kt) => {
        ...
        const day = type === 'person' ? parseInt(list[0] + list[1], 10) : parseInt(list[0] - 4 + list[1], 10);
        ...
        return {
            years: existanceInYears(birthday),
            msAge: now - birthday,
            birthdayToday,
        };
    }
    ...
    const isValid = (kt) => {
        ...
        currentRadtala = parseInt(list[6] + list[7], 10);
    ...
    const calculateVartala = (list, incrementRadtala) => {
        ...
        const radtala = ('0' + currentRadtala.toString()).slice(-2).split('');
